...
const onRight = () => {
  const { start, stop } = useScrollTo({
    el: scrollbarRef,
    to: scrollbarRef.value!.scrollWidth,
    directions: "scrollLeft",
    duration: 2000,
    callback: () => {
      console.log("已滚动到最右侧");
...
<template>
  <naive-theme>
    <div
      ref="scrollbarRef"
      class="w-[600px] overflow-x-scroll flex bg-[#f8fafc] dark:bg-[#1e293b] rounded-lg"
    >
      <div
        v-for="(text, index) in 66"
        :key="index"
        class="shrink-0 text-center px-[20px] pt-[20px]"
      >
